...
typedef uint8_t LIS2DH_IntPinConf_t;
typedef uint8_t LIS2DH_Axis_t;
typedef uint8_t LIS2DH_Int1Conf_t;
...
#define LIS2DH_STATUS_AUX_TOR 0x40
#define LIS2DH_STATUS_AUX_TDA 0x04
...
extern uint8_t AccelReadRegister(uint8_t address, uint8_t reg);
...
status_t LIS2DH_GetClickResponse(uint8_t *val);
status_t LIS2DH_GetTempRaw(int8_t *val);
...
extern void DelayLis2dhMsTime(uint16_t delay_time);
extern void GsensorInterruptRoutine(void);
extern void GsensorIntProcess(void);
extern void GsensorStart(void);
extern void GsensorStop(void);
extern void GsensorGetRawAxes(void);
